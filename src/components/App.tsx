...
class App extends React.Component<any, AppState> {
  ...
  mapClick(e: LocationEvent, feature: any): void {
    this.setState({gridCell: [feature.geometry.coordinates[0][0][1],feature.geometry.coordinates[0][0][0]], mapRendered: false}, () => {console.log("ya:  ", this.state.gridCell)});
  }
  ...
  render () {
    return (
      <div className="App">
        <Nav navBtnClick={this.navBtnClick.bind(this)} mapRendered={this.state.mapRendered} gridCell={this.state.gridCell} ></Nav>
        { this.state.mapRendered ? 
          (<LeafletMap clickHandler={this.mapClick.bind(this)} mapRendered={this.state.mapRendered} gridCell={this.state.gridCell}/>)
        :
          (<VizContainer gridCell={this.state.gridCell} />)
        }
      </div>
    );
  };
